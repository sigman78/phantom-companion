    ...
    {#if activation && ownClassCard}
      <div class="actions-panel">
        ...
        <div class="actions-group-header">
          <span class="ag-type">Species</span>
        ...
        <div class="actions-list">
          {#each activation.speciesCard.Actions as step}
            <div class="action-row">
              ...
              {#if step.Effect}<span class="action-note">{step.Effect}</span>{/if}
              {#if step.Target}<span class="action-note">{step.Target}</span>{/if}
              {#if step.Type}<span class="action-tag">{step.Type}</span>{/if}
            </div>
          {/each}
        </div>
...
<style>
  ...
  .ag-ap   { font-size: 12px; color: var(--color-accent); flex-shrink: 0; }
  ...
  .action-val  { font-size: 16px; font-weight: bold; color: var(--color-text); flex-shrink: 0; }
  .action-note { font-size: 13px; color: var(--color-text-dim); font-style: italic; }
  .action-tag {
    font-size: 10px;
    text-transform: uppercase;
